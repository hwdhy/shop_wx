...
Page({
  ...
  getSearchKeyword() {
    ...
    util.request(api.SearchIndex).then(function (res) {
      if (res.errno === 0) {
        that.setData({
          historyKeyword: res.data.history_keyword_list,
          defaultKeyword: res.data.default_keyword,
          hotKeyword: res.data.hot_keyword_list
        });
      }
    });
  },
    ...
    util.request(api.GoodsList, { keyword: that.data.keyword, page: that.data.page, size: that.data.size, sort: that.data.currentSortType, order: that.data.currentSortOrder, categoryId: that.data.categoryId }).then(function (res) {
      ...
      if (res.errno === 0) {
        that.setData({
          ...
          filterCategory: res.data.filter_category,
